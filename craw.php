...
while ($check_page) {
    ...
    $html->load_file($URL);
    getProductLink($URL);
    if ($next = $html->find('a[class=next page-number]', 0)) {
        $URL = $next->href;
        $html->clear();
        $page += 1;
    } else {
...
//getProductLink("https://vnsup.com/danh-muc/thiet-bi-chuyen-doi-gateway/gateway-dinstar/");
var_dump($crawled_link);
getProductInformation("https://vnsup.com/san-pham/cong-dinstar-mtg3000t/");
var_dump($data_crawed);
...
function getProductLink($URL)
{
    ...
    $html = new simple_html_dom();
    $html->load_file($URL);
    foreach ($html->find('div[class=product-small box]') as $element) {
        $link = $element->find("a",0);
        $product_link = $link->getAttribute('href');
...
function getProductInformation($URL)
{
    ...
    $html->load_file($URL);
    $product_infomation_div     = $html->find('div[class=product-info summary entry-summary col col-fit product-summary text-left]', 0);
    // $product_image_a            = ($html->find('figure[class=woocommerce-product-gallery__image slide first is-selected]')->find("a") != null) ? $html->find('figure[class=woocommerce-product-gallery__image slide first is-selected]')->find("a") : '' ;
    
    // Hai anh da lam toi day (ten, link, anh, long, short)
    $product_image_a            = $html->find('div[class*=woocommerce-product-gallery__image]');

    $product_image_source       = (isset($product_image_a)) ? $product_image_a->href : '';
    $product_name_h1            = $product_infomation_div->find('h1[product-title product_title entry-title]', 0);
    $product_name               = isset($product_name_h1) ? $product_name_h1->plaintext : '';
    $product_description        = $html->find('div[class=product-footer]',0)->innertext;
